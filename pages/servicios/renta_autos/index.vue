<template>
  ...
  <v-container fluid>
    ...
    <DesplegarHorarios v-model="dialogos.horarios" :negocio.sync="rentas.seleccionado" />
...
export default {
  ...
  computed: {
    ...
    listado() {
      ...
      if(this.helpers.filtro_fecha || this.helpers.filtro_hora){
        ...
        listado_total.filter(renta => {
          ...
          if(this.helpers.filtro_hora && this.helpers.filtro_fecha && this.helpers.hora_planeada &&
            ...
            if(this.VerificarHoraFiltro(this.helpers.hora_planeada, renta) &&
              this.VerificarFechaFiltro(this.helpers.fecha_planeada, renta)){
            ...
            if(this.VerificarFechaFiltro(this.helpers.fecha_planeada, renta)){
      ...
      lista.forEach( async renta => {

        this.markers.push({position: {lat: +renta.lat, lng: +renta.lng}, negocio: renta})
  ...
  data(){
    return{
      ...
      icon_marker: {
        url: "https://static.thenounproject.com/png/386502-200.png",
  ...
  methods: {
    ...
    async ObtenerRentaAutos(){
      ...
      await this.$api.post("/negocios/categoria", { categoria: "RC" }).then( data => {
        ...
        this.rentas.listado.forEach( async renta => {
          ...
          await this.$api.post("/horario/negocio", {negocioId: renta.id}).then(data => {

            renta.horarios = data

            renta.horarios.sort(function (a, b) {
              return a.dia - b.dia
            })

          })

          await this.$api.post("/productos", { id: renta.id }).then(data => {
            ...
            try {
              ...
              renta.rango = [+data[0].valor, +data[data.length - 1].valor]
    ...
    MostrarDialogoHorarios(negocio){/**********************************************************/
      ...
      this.rentas.seleccionado = Object.assign({}, negocio)
    ...
    async ObtenerTagPool(){
      ...
      await this.$api.post("/tags/find", { categoria: 'RC' }).then( data => {
      ...
      const contenido =
        (`<div class="card">
          ...
          <div class="card-content">
            ...
            <a href="/servicios/renta_autos/${mk.negocio.id}" target="_blank" style="text-decoration: none;">
