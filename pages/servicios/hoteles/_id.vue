    ...
    <v-row>
      ...
      <v-col cols="1" align-self="center" class="hidden-sm-and-down">
        ...
        <v-avatar
          ...
          tile
        >
          <v-img v-if="hotel.img" :src="hotel.img" style="border-radius:10px;" />
            ...
            <v-list-item-title>
              <v-row class="py-4 px-3">
                ...
                <div class="grey--text mt-1 text-wrap">
                  ...
                  <v-icon class="mx-1" small color="black"> fa fa-map-marker-alt </v-icon>{{ hotel.direccion }} |
                  <v-icon class="mx-1" small color="black"> fa fa-phone </v-icon> (+502) {{ FormatTelefono() }} |
                  <v-icon class="mx-1" small color="black"> fa fa-clock </v-icon> {{ $moment(hotel.abre, "HH:mm:ss").format('h:mm a') }} -
                  {{ $moment(hotel.cierra, "HH:mm:ss").format('h:mm a')  }} (<span :class="VerificarHora() === 'Cerrado' ?
                                                                                    'red--text' : 'green--text'">
                    {{ VerificarHora()}}
                  </span>)
          ...
          <v-card-text class="pa-3">
            ...
            <v-list-item class="ml-n4">
              <v-list-item-content>
                ...
                <v-list-item-subtitle class="mt-2 text-wrap">{{ hotel.descripcion }}</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
              ...
              <v-col cols="12">
                ...
                <v-list class="ml-n4 mt-n2">
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-content>
                      <v-list-item-title>
                        {{ $moment(hotel.abre, "HH:mm:ss").format('h:mm a') }} -
                        {{ $moment(hotel.cierra, "HH:mm:ss").format('h:mm a')  }} (<span :class="VerificarHora() === 'Cerrado' ?
                                                                                    'red--text' : 'green--text'">
              ...
              <v-col cols="12">
                ...
                <v-list class="ml-n4 mt-n2">
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-content>
                      <v-list-item-title class="text-wrap">Dirección: {{ hotel.direccion }}</v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
...
import Valoracion from "@/components/hoteles/Valoracion";
  ...
  methods: {
    ...
    async EnviarMensaje(){
      ...
      if(this.$store.state.negocios && this.$store.state.negocios.length > 0){
        negocioFound = this.$store.state.negocios.find( n => n.id === this.hotel.id );
      }
      ...
      try {
        ...
        if(login){
          ...
          if(negocioFound && negocioFound.id > 0){
            ...
            await this.$api.post("/usuario/info", { id: this.hotel.usuarioId })
              .then(async data => {
                let encargado = data
                try {

                  const chatsRef = this.$fire.database.ref(
                    'Chats/'+'chat' + this.auth.id+"-"+encargado.id + '/'+'idNegocio'+ this.hotel.id
                  )
                  ...
                  Axios.get(chatsRef.toString() + '.json').then(async response => {
                    ...
                    if(!response.data){
                      let chat = {
                        ...
                        key_negocio: 'idNegocio'+this.hotel.id,
                        ultimoMensaje: ''
                      }
                    ...
                    this.$router.push({path: '/usuario/mensajes?id=' + this.hotel.id })
    ...
    async ObtenerHotel(){
      ...
      await this.$api.post("/negocios/categoria", { categoria: "H" }).then( data => {
        ...
        this.marker.position.lat = +this.hotel.lat
        this.marker.position.lng = +this.hotel.lng
        this.center.lat = +this.hotel.lat
        this.center.lng = +this.hotel.lng
        this.$forceUpdate()
    ...
    async ObtenerTags(){
      ...
      this.hotel.tags = await this.$api.post("/tags/negocio", { negocioId: this.hotel.id })
    ...
    GaleriaImagenes(){
      ...
      this.index = null
      this.$router.push({ path: '/servicios/hoteles/galeria?id='+this.hotel.id })

    },

    VerificarHora(){

      if(this.hotel && this.hotel.abre && this.hotel.cierra){

        var format = 'hh:mm:ss'
        var time = this.$moment(this.$moment(),format),
          beforeTime = this.$moment(this.hotel.abre, format),
          afterTime = this.$moment(this.hotel.cierra, format);
    ...
    FormatTelefono(){
      ...
      if(this.hotel.telefono){

        return this.hotel.telefono.substring(0, 4) + " " + this.hotel.telefono.substring(4)
    ...
    Regresar(){
      ...
      else if(!this.$nuxt.context.from.path.includes("galeria") && !this.$nuxt.context.from.path.includes("hoteles")){
        path = '/'
      }
      else{
