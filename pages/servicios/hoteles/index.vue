<template>
  ...
  <v-container fluid>
    ...
    <DesplegarHorarios v-model="dialogos.horarios" :negocio.sync="hoteles.seleccionado" />
...
export default {
    ...
    listado() {
      ...
      if(this.helpers.filtro_rango){
        ...
        this.hoteles.listado.forEach(hotel => {
          ...
          if(this.VerificarRangoPrecios(hotel.rango)){

            listado_total.push(hotel)
        ...
        listado_total = [...this.hoteles.listado]
      }

      if(this.helpers.filtro_fecha || this.helpers.filtro_hora){

        listado_total.filter(hotel => {

          if(this.helpers.filtro_hora && this.helpers.filtro_fecha && this.helpers.hora_planeada &&
            this.helpers.fecha_planeada){

            if(this.VerificarHoraFiltro(this.helpers.hora_planeada, hotel) &&
              this.VerificarFechaFiltro(this.helpers.fecha_planeada, hotel)){
            ...
            if(this.VerificarFechaFiltro(this.helpers.fecha_planeada, hotel)){
      ...
      lista.forEach( async hotel => {

        this.markers.push({position: {lat: +hotel.lat, lng: +hotel.lng}, negocio: hotel})
  ...
  data(){
    return{
      ...
      icon_marker: {
        url: "https://cdn3.iconfinder.com/data/icons/maps-and-pins-4/512/map_pin_destination_location_adress_hotel_bed-512.png",
  ...
  methods: {
    ...
    async ObtenerHoteles(){
      ...
      await this.$api.post("/negocios/categoria", { categoria: "H" }).then( data => {
        ...
        this.hoteles.listado.forEach(async hotel => {
          ...
          await this.$api.post("/horario/negocio", {negocioId: hotel.id}).then(data => {

            hotel.horarios = data

            hotel.horarios.sort(function (a, b) {
              return a.dia - b.dia
            })
    ...
    MostrarDialogoHorarios(negocio){/**********************************************************/
      ...
      this.hoteles.seleccionado = Object.assign({}, negocio)
    ...
    async ObtenerTagPool(){
      ...
      await this.$api.post("/tags/find", { categoria: 'H' }).then( data => {
      ...
      const contenido =
        (`<div class="card">
          ...
          <div class="card-content">
            ...
            <a href="/servicios/hoteles/${mk.negocio.id}" target="_blank" style="text-decoration: none;">
