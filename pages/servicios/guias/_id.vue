<template>
  ...
  <v-container fluid>
            ...
            <v-list-item-title>
              <v-row class="py-4 pl-2">
                ...
                <div class="grey--text mt-1 ml-1">
                  {{ guia.totalValoraciones }} valoraciones |
                  ...
                  Se encuentra a <span class="font-weight-bold">
                    {{ CalcularDistancia(guia.lng, guia.lat) }} km
                      </span> de ti
                </div>
    ...
    <ListadoHorarios v-model="dialogos.horarios" :negocio.sync="guia" />
...
import Valoracion from "@/components/guias/Valoracion";
...
export default {
  ...
  methods: {
    ...
    async ObtenerHorarios(){
      ...
      this.guia.horarios = await this.$api.post("/horario/negocio", { negocioId: +this.$route.params.id })

      this.guia.horarios.sort(function (a, b) {
        return a.dia - b.dia
      })
    ...
    async EnviarMensaje(){
      ...
      if(this.$store.state.negocios && this.$store.state.negocios.length > 0){
        negocioFound = this.$store.state.negocios.find( n => n.id === this.guia.id );
      }
      ...
      try {
        ...
        if(login){
          ...
          if(negocioFound && negocioFound.id > 0){
            ...
            await this.$api.post("/usuario/info", { id: this.guia.usuarioId })
              .then(async data => {
                let encargado = data
                try {

                  const chatsRef = this.$fire.database.ref(
                    'Chats/'+'chat' + this.auth.id+"-"+encargado.id + '/'+'idNegocio'+ this.guia.id
                  )
                  ...
                  Axios.get(chatsRef.toString() + '.json').then(async response => {
                    ...
                    if(!response.data){
                      let chat = {
                        ...
                        key_negocio: 'idNegocio'+this.guia.id,
                        ultimoMensaje: ''
                      }
                    ...
                    this.$router.push({path: '/usuario/mensajes?id=' + this.guia.id })
    ...
    async ObtenerValoraciones(){
      ...
      this.guia.totalValoraciones = this.valoraciones.length
      this.guia.puntuacionAvg = (this.valoraciones.length > 0) ? valoracionesAvg / this.valoraciones.length : 0
    ...
    Regresar(){
      ...
      if(this.$nuxt.context.from.path.includes("galeria")){
        path = '/servicios/guias'
      }
      else if(!this.$nuxt.context.from.path.includes("galeria") && !this.$nuxt.context.from.path.includes("guias")){
        path = '/'
      }
      else{
